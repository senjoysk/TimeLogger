...
ALTER TABLE user_settings ADD COLUMN prompt_start_hour INTEGER DEFAULT 8 CHECK (prompt_start_hour >= 0 AND prompt_start_hour <= 23);

-- prompt_start_minute カラムを追加（開始時刻の分）
ALTER TABLE user_settings ADD COLUMN prompt_start_minute INTEGER DEFAULT 30 CHECK (prompt_start_minute IN (0, 30));

-- prompt_end_hour カラムを追加（終了時刻の時）
ALTER TABLE user_settings ADD COLUMN prompt_end_hour INTEGER DEFAULT 18 CHECK (prompt_end_hour >= 0 AND prompt_end_hour <= 23);

-- prompt_end_minute カラムを追加（終了時刻の分）
ALTER TABLE user_settings ADD COLUMN prompt_end_minute INTEGER DEFAULT 0 CHECK (prompt_end_minute IN (0, 30));
