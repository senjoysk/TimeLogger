...
if flyctl deploy --app "$STAGING_APP_NAME" --config fly-staging.toml; then
    ...
    echo "=== Recent Logs ==="
    flyctl logs --app "$STAGING_APP_NAME" | head -50 || echo "ログ取得失敗"
...
echo "   🌐 https://$STAGING_APP_NAME.fly.dev"
echo "   💚 ヘルスチェック: https://$STAGING_APP_NAME.fly.dev/health"
echo ""
echo "2. Discord Bot の重要機能テスト"
echo "   • !cost - API使用量レポート"
echo "   • !summary - 日次サマリー"
echo "   • !timezone - タイムゾーン設定"
echo ""
echo "3. 問題なければmainブランチへマージ"
echo "   → Production環境への自動デプロイ"
echo ""
echo "🔍 トラブルシューティング:"
echo "   • ログ確認: flyctl logs --app $STAGING_APP_NAME"
echo "   • マシン状態: flyctl machines list --app $STAGING_APP_NAME"
echo "   • アプリ状態: flyctl status --app $STAGING_APP_NAME"