...
### 2.5 未登録活動時間の記録（`/gap`コマンド）

ユーザーが活動を記録し忘れた時間帯（ギャップ）を検出し、対話形式で簡単にログを追加できるようにする機能。

-   **コマンド**: `/gap` - 活動記録がない時間帯を検索し、記録を促す。
-   **ギャップの定義**: ユーザーの活動ログの間隔が15分以上空いている箇所。
-   **検出対象時間帯**: 
    -   コマンド実行日の午前7:30から午後6:30（18:30）まで。
    -   上記時間帯以外は意図的な非活動時間とみなし、検出対象外とする。
-   **出力形式**:
    -   検出されたギャップを時間のリストとしてDiscordの埋め込みメッセージ（Embed）で表示。
    -   各ギャップに対して、活動内容を追記するためのボタンを設置。
-   **活動の追加**:
    -   ユーザーがボタンを押すと、モーダル（入力フォーム）が表示される。
    -   ユーザーがモーダルに活動内容を入力して送信すると、その内容が自動でログに登録される。
    -   登録されるログのタイムスタンプは、ギャップ期間の開始時刻とする。
-   **動作フロー**:
    1. ユーザーが`/gap`コマンドを実行する。
    2. システムは、コマンド実行者の当日の活動ログを取得する。
    3. 午前7:30から午後6:30の範囲で、以下のギャップを検出する。
        - ケース1: 7:30から最初のログまでの間隔が15分以上ある場合。
        - ケース2: ログとログの間の間隔が15分以上ある場合。
        - ケース3: 最後のログから現在時刻（ただし18:30を上限とする）までの間隔が15分以上ある場合。
    4. 検出結果に応じて応答する。
        - ギャップがない場合: 「✅ 7:30〜18:30の間に15分以上の記録の空白はありませんでした。」
        - ギャップがある場合: 時間帯のリストと各時間帯の記録ボタンを表示。
